...
def plot_results(G_full, results, metrics_history):
    ...
    plt.axhline(y=1.3, color='r', linestyle='--',
                label='1D→2D Threshold (1.3)')
    plt.axhline(y=1.15, color='g', linestyle='--',
                label='2D→3D Threshold (1.15)')
    plt.axhline(y=1.01, color='b', linestyle='--',
                label='3D→4D Threshold (1.01)')
    plt.axhline(y=1.00, color='m', linestyle='--',
                label='4D→5D Threshold (1.00)')
    ...
    if results.get('4D') is not None:
        plt.axvline(x=results.get('4D'), color='b', linestyle=':',
                    label=f'3D→4D Spacetime at n={results.get("4D")}')
    if results.get('5D') is not None:
        plt.axvline(x=results.get('5D'), color='m', linestyle=':',
                    label=f'4D→5D at n={results.get("5D")}')
    ...
    plt.axhline(y=2.2, color='g', linestyle='--',
                label='2D→3D Threshold (2.2)')
    plt.axhline(y=3.0, color='b', linestyle='--',
                label='3D→4D Threshold (3.0)')
    plt.axhline(y=4.0, color='m', linestyle='--',
                label='4D→5D Threshold (4.0)')
    if results.get('2D') is not None:
        plt.axvline(x=results.get('2D'), color='r', linestyle=':',
                    label=f'1D→2D at n={results.get("2D")}')
    if results.get('3D') is not None:
    ...
    if results.get('4D') is not None:
        ...
        ax.set_zlabel('Z (Eigenvector 3)')
        ax.set_title(
            f'3D Spatial Embedding at n={results["4D"]} (3D→4D Spacetime)')
        plt.legend()
...
def print_validation(G, results):
    ...
    if results["3D"] is not None:
        ...
        print(
            f"  λ₃/λ₂ = {r2:.3f} (> 1.15: {'Valid' if r2 > 1.15 else 'Invalid'})")
        print(
            f"  R_n = {R_n:.3f} (> 2.2: {'Valid' if R_n > 2.2 else 'Invalid'})")
        print(
    ...
    if results["4D"] is not None:
        ...
        embedding_4D = compute_embedding_spacetime(G_4D, spatial_dim=3)
        euclidean_dists = np.sqrt(
            np.sum((embedding_4D[:, 1:4] - embedding_4D[:, 1:4][:, np.newaxis])**2, axis=2))
        np.fill_diagonal(d_ij, 0)
        distortion = np.mean((euclidean_dists - d_ij)**2 / (d_ij**2 + 1e-16))
        print(f"\n3D→4D Spacetime Transition at n={results['4D']}:")
        print(
            f"  λ₄/λ₃ = {r3:.3f} (> 1.01: {'Valid' if r3 > 1.01 else 'Invalid'})")
        print(
            f"  R_n = {R_n:.3f} (> 3.0: {'Valid' if R_n > 3.0 else 'Invalid'})")
        print(
            f"  Energy Functional (Axiom V): E_n = {E_n:.3f} (>= 0: {'Valid' if E_n >= 0 else 'Invalid'})")
        print(
            f"  Spectral Gaps (Annex H): λ₁ = {gap1:.3f}, λ₂-λ₁ = {gap2:.3f}, λ₃-λ₂ = {gap3:.3f}, λ₄-λ₃ = {gap4:.3f}, λ₅-λ₄ = {gap5:.3f}")
...
def simulate(n_max):
    ...
    with tqdm(range(1, n_max + 1),
        ...
        for n in pbar:
            ...
            t2d, t3d, t4d, t5d, r1, r2, r3, r4 = check_transitions(
                G, d_min, l_info, R_n)
    ...
    plot_results(G_full, results, (ns, d_mins,
                 l_infos, R_ns, r1s, r2s, r3s, r4s))
